...
docker buildx build --tag="ghcr.io/superorbital/rails-docker-parallel-example:${BUILDKITE_COMMIT}" --platform=linux/arm64 --push --metadata-file="$(pwd)/metadata.json" --build-arg REPO="${BUILDKITE_REPO}" .
